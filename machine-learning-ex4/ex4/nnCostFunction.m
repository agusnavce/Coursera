...
%         computed in ex4.m
            ...
            % Obtaining the value of the cost function without regularization

            suma = (-y).*log(a3)-(1-y).*log(1-a3);
            ...
            J=((1/m).*sum(sum(suma)))+(lambda/(2*m)).*(sum(sum(Theta1s.^2))+sum(sum(Theta2s.^2)));
